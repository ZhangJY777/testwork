...
func (t *Application) SetVVV(args []string) (string, error) {
...
func (t *Application) SetVV(args []string) (string, error) {
	request := channel.Request{ChaincodeID: t.SdkEnvInfo.ChaincodeID, Fcn: args[0], Args: [][]byte{[]byte(args[1]), []byte(args[2]), []byte(args[3]), []byte(args[4]),[]byte(args[5]),[]byte(args[6])}}
	response, err := t.SdkEnvInfo.Client.Execute(request)
	if err != nil {
		return "", err
	}
